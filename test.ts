...
class Bat {
    ...
    looks() {
        console.log(
            "Bat " + this.name + " is " + (this.color ?? "unknown color")
        )
    }
}

const taphozous : Bat = new Bat("taphozous", "microbat", "ultrasonic", 5, "brown");
const pippistrelle : Bat = new Bat("pippistrelle", "microbat", "ultrasonic", 5);
const myotis : Bat = new Bat("myotis", "microbat", "ultrasonic", 5);
const flyingFox : Bat = new Bat("flying fox", "macrobat", "subultrasonic", 1000, "black");
